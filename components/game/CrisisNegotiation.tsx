...
export const CrisisNegotiation: React.FC<CrisisNegotiationProps> = ({ metric, onResult }) => {
  ...
  const initialPrompts: Record<string, string> = {
    ...
    civility: "现在的村子还有点人气吗？到处都是赌博的、混日子的！老祖宗留下的规矩全毁在你手里了！"
  };

  useEffect(() => {
    if (stage === 'chat') {
      setMessages([{ role: 'model', text: initialPrompts[metric] || initialPrompts['civility'] }]);
    }
  }, [stage, metric]);
  ...
  if (stage === 'decision') {
    return (
      ...
      >
        ...
        <h2 className="text-white text-2xl font-song font-bold mb-4">危机状态：{metricNameMap[metric] || metricNameMap['civility']}失控</h2>
        <p className="text-white/70 font-fangsong text-lg mb-10 leading-relaxed">
          {metricNameMap[metric] || metricNameMap['civility']}指标已降至临界点以下。大批情绪激动的群众正聚集在村委会门口，事态极其严峻！
        </p>
      ...
      <div className="w-full max-w-md bg-gov-paper rounded-2xl overflow-hidden shadow-2xl flex flex-col h-[85vh] relative">
        <div className="bg-gov-red text-white p-4 flex justify-between items-center shadow-lg">
          <div className="flex flex-col">
            ...
            <h2 className="font-song text-lg font-bold">现场协调：{metricNameMap[metric] || metricNameMap['civility']}</h2>
          </div>
